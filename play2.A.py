#!/usr/bin/env python3
"""
Query Matcher - Iteration 2.A (Cosine Similarity - No Stop Words)
Experimental: Removes stop word filtering to preserve critical words
"""
...
# Import our modified classifier
...
class CosineClassifierNoStopWords:
    def __init__(self):
        # KEY CHANGE: stop_words=None instead of 'english'
...
def display_welcome():
    ...
    welcome_text.append("QUERY MATCHER v2.A", style="bold yellow")
    welcome_text.append(" - No Stop Words", style="bold cyan")
    
    panel = Panel(
        "[magenta]Experimental: Cosine Similarity WITHOUT Stop Word Removal[/magenta]\n"
        "[dim]Preserves critical words like 'can't', 'won't', 'my', etc.[/dim]",
        title=welcome_text,
...
def quick_match(classifier):
    ...
    console.print("[bold magenta]Quick Match Mode - No Stop Words[/bold magenta]")
    ...
    while True:
        ...
        with console.status("[magenta]Finding most similar example...[/magenta]"):
            try:
                ...
                # Warn if zero similarity (should be less common now)
...
def run_full_test(classifier):
    ...
    if zero_sim_wrong > 0:
        ...
        console.print("[dim]Even without stop words, some queries still have no matches[/dim]")
...
def show_about():
    ...
    about_text = """
[bold magenta]Iteration 2.A: No Stop Words Experiment[/bold magenta]

[yellow]Key Change:[/yellow]
• Removed stop_words='english' from TfidfVectorizer
• Now preserves words like: can't, won't, my, is, the, etc.
• Added sublinear_tf=True for better term frequency scaling

[yellow]Hypothesis:[/yellow]
The zero similarity issue occurs because critical words like "can't"
in "I can't login" are filtered out as stop words, leaving no 
meaningful terms to match against training examples.

[yellow]Expected Improvements:[/yellow]
• Fewer zero similarity cases
• Better matching for queries with negations
• Preserved context from small but important words

[yellow]Trade-offs:[/yellow]
• Larger vocabulary size (more memory)
• Potentially noisier similarity scores
• Common words might reduce discrimination

[yellow]Test Focus:[/yellow]
• Check if "I can't login" now matches correctly
• Monitor zero similarity statistics
• Compare overall accuracy with original version
    """
    
    console.print(Panel(about_text, title="[green]About No Stop Words Approach[/green]", 
                       border_style="green"))
    Prompt.ask("\n[dim]Press Enter to continue[/dim]")
...
def main():
    ...
    console.print("[magenta]Initializing Cosine Similarity Classifier (No Stop Words)...[/magenta]")
    classifier = CosineClassifierNoStopWords()
    ...
    while True:
        ...
        if choice == "0":
            console.print("[yellow]Thanks for using Query Matcher v2.A! Goodbye! 👋[/yellow]")
            break
        elif choice == "1":
